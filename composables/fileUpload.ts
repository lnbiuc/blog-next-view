...
export function upload(filePath: string, filename: string): string {
  ...
  cos.putObject({
    Bucket: process.env.BUCKET_NAME ? process.env.BUCKET_NAME : '', /* 填入您自己的存储桶，必须字段 */
    Region: process.env.REGION_NAME ? process.env.REGION_NAME : '', /* 存储桶所在地域，例如 ap-beijing，必须字段 */
